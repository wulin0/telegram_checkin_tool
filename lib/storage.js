...
const Storage = (() => {
  ...
  /**
   * 批量更新签到结果（性能优化：一次 read + 一次 write 替代 N 次）
   * @param {Array<{id: string, updates: object}>} batch
   */
  async function batchUpdateCheckinResults(batch) {
    ...
    for (const { id, updates } of batch) {
      const idx = groups.findIndex(g => g.id === id);
      if (idx === -1) continue;
      // 合并更新（跳过 undefined 字段）
      for (const [key, val] of Object.entries(updates)) {
        if (val !== undefined) groups[idx][key] = val;
      }
      history.push({
        groupId: id,
        groupName: groups[idx].name,
        keyword: groups[idx].keyword,
        timestamp: now,
        result: updates.lastResult || null
      });
    }
